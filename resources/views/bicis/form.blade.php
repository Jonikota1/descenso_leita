        ...
        <form class="w-full max-w-lg" method="POST" action="{{ $route }}">
            ...
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full px-3">
                    ...
                    <input name="stock" value="{{ old('stock') ?? $bici->stock }}" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="stock" type="number" min='0' required>
                    <p class="text-gray-600 text-xs italic">{{ __("Stock del objeto") }}</p>
                    @error("stock")
                    <div class="border border-red-400 rounded-b bg-red-100 mt-1 px-4 py-3 text-red-700">
                        {{ $message }}
                    </div>
