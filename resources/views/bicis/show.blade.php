...
<div class="col-sm-8">
   ...
   <p><b>Stock: </b> {{$bicis->stock}} </p>
   <p><b>Precio: </b> {{$bicis->precio}}</p>

   <div class="btn-group btn-group-justified">
   @auth
   @if(auth()->user()->roles()->find(1))
<a href="{{route('bicis.edit',['bici'=>$bicis->id])}}"><button type="button" class="btn btn-success">Editar</button></a>
...
<a href="#" class="text-red-400" onclick="event.preventDefault(); 
 ...
 <form id="delete-bicis-{{$bicis->id}}-form" action="{{route('bicis.destroy',['bici'=>$bicis->id])}}" method="POST" class="hidden">
   ...
   <a href="{{route('bicis.index')}}"> <button type="button" class="btn btn-dark text-white">Catálogo</button></a>
   </div>
</div>
</div>
@endsection